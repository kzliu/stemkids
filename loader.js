...
conn.query('CREATE TABLE IF NOT EXISTS class_attendance (user_id INTEGER, class_id INTEGER, course_id TEXT, active INTEGER, PRIMARY KEY (user_id, class_id), FOREIGN KEY (class_id) REFERENCES classes(class_id), FOREIGN KEY (user_id) REFERENCES user_info(user_id));').on('error', console.error).on('end', function(){
